...
{
public:
    explicit SerialPacket(quint8 addr = 0, quint8 cmd = 0, quint8 len = 0);
 ...
 * Реализация информационного обмена по каналу RS-485
 */
...
public:
    ...
    quint16 computeCRC16(const SerialPacket &pack);
...
private:
    ...
    ReceiverState checkAddress(quint8 byte);
    ReceiverState checkCmd(quint8 byte);
    ReceiverState checkLen(quint8 byte);
    ReceiverState checkData(quint8 byte);
    ReceiverState doSomeStuff(ReceiverState state, quint8 byte);
    ReceiverState checkCrc(quint8 byte);
    ReceiverState finalCrcCheck(quint8 byte);
    ...
    quint16 crc16_ccitt (quint16 sum, quint8 *buf, quint16 len);
    quint16 crc16_ccitt_byte (quint16 sum, quint8 byte);
...
private:
    ...
    const quint16 CRC16_CCITT_INIT = 0xffff;  /* Initial checksum value */
